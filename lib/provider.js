...
async function getInteraction(req, res) {
  ...
  //console.log("oidc-debug: here is the req, res: ", JSON.stringify(req), JSON.stringify(res))
  const ctx = this.app.createContext(req, res);
  console.log("oidc-debug: here is the context", JSON.stringify(ctx))
  //console.log(`oidc-debug: sshandler will be called with ${JSON.stringify(ctx.cookies)} and ${JSON.stringify(instance(this).configuration)}`)
  const id = ssHandler.get(
    ctx.cookies,
    this.cookieName('interaction'),
    instance(this).configuration('cookies.short'),
  );
  console.log("oidc-debug: here is the id", JSON.stringify(id))
  if (!id) {
...
class Provider extends events.EventEmitter {
  constructor(issuer, setup) {
    ...
    instance(this).Account = setup.Account;
