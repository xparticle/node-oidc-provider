...
module.exports = async function resumeAction(whitelist, resumeRouteName, ctx, next) {
  ...
  console.log("oidc-provider:resume: Cookie ID is: ", cookieId, ctx.oidc.uid)
  ...
  console.log("oidc-provider:resume: InteractionSession is: ",JSON.stringify(interactionSession))
  const {
    result,
    params: storedParams = {},
    signed = [],
  ...
  console.log("oidc-provider:resume: result check: ",JSON.stringify(result))
  if (result && result.login) {
    const {
      remember = true, account, ts: loginTs, amr, acr,
    } = result.login;
    console.log("oidc-provider:resume: About to call loginAccount with: ", JSON.stringify({
