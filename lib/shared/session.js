...
module.exports = async function sessionHandler(ctx, next) {
  console.log("Shared-session: context", ctx)
  console.log("Shared-session: here is the proxy target:", JSON.stringify(await ctx.oidc.provider.Session.get(ctx)))
  ctx.oidc.session = new Proxy(await ctx.oidc.provider.Session.get(ctx), {
    set(obj, prop, value) {
      console.log("Shared-session: (obj,prop,value)", obj, prop, value)
      switch (prop) {
        case 'touched':
